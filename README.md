...
### Дерево DOM
DOM нужен для того, чтобы манипулировать странице. Дом это объектная модель. 
Любой узел является объектом и через JavaScript мы можем изменить его свойство
...
```
Получается, что с одной стороны у DOM элементов есть свой свойтва, как у обычных элементо с другой стороны есть аттрибуты, которые можно устанавливать с помощью методов.
